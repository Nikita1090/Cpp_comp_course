...
class Parser {
    ...
    //RD procedures
    void P();
    void D(token_type type, std::vector<Ident>& tid = TID, bool is_struct = false);
    void D1(std::vector<Ident>& tid = TID, bool is_struct = false);//Variable descriptions
    ...
    void B1(bool is_cycle = false);//Checking for cycle
    void G(Token token, bool is_cycle = false);
    void S(Token token);
    void E(Token token, bool not_object = false, bool is_write = false);
    void E1();
    void E2();
    void E3();
    void E4();
    void T();
    void F(bool is_minus = false, bool is_not = false);

    void Declare(token_type variable_type, std::vector<Ident>& tid = TID);
    void declareStruct(int value = 0);
    ...
    void checkMinus();
    void eqType();
    void eqBool();//Context conditions
    void checkIdInRead();
...
void Parser::D(token_type variable_type, std::vector<Ident>& tid, bool is_struct)
{
    ...
    while(current_type == Comma) {
        ...
        if(current_type == Assign) {
            ...
            poliz.push_back(Token(Assign));
            poliz.push_back(Token(Semicolon, 1));
            getToken(tid);
        }
    }
...
void Parser::B(bool is_cycle)
{
    ...
    //For-loop
        ...
        l1 = poliz.size();
        E(current_token);
        eqBool();//if, while context conditions
        if(current_type != Semicolon)
            throw current_token;
        getToken();
        l2 = poliz.size();
